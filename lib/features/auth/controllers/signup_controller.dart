...
class RegisterController {
  ...
  Future<void> handleEmailPasswordRegistration({
    ...
    try {
      ...
      print('RegisterController: API Response = ${response.body}, statusCode=${response.statusCode}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final updatedAuthModel = AuthModel(
          fullname: data['data']['fullname'] ?? data['data']['name'] ?? fullName.trim(),
        ...
        print('RegisterController: Storing tokens and user data - fullname=${updatedAuthModel.fullname}, accessToken=$accessToken');
        await _authController.storeTokens(accessToken, refreshToken);
        await _authController.storeUserData(updatedAuthModel);

        print('RegisterController: Updating AuthProvider with fullname=${updatedAuthModel.fullname}');
        Provider.of<AuthProvider>(context, listen: false).setUser(updatedAuthModel);
  ...
  Future<void> handleGoogleSignIn({
    ...
    try {
      ...
      if (userCredential.user != null) {
        ...
        print('RegisterController: Storing user data - fullname=${authModel.fullname}, token=${authModel.token}');
        await _authController.storeUserData(authModel);
        if (googleAuth.accessToken != null) {
          print('RegisterController: Storing Google tokens - accessToken=${googleAuth.accessToken}');
          await _authController.storeTokens(googleAuth.accessToken!, googleAuth.accessToken!);
        }

        print('RegisterController: Updating AuthProvider with fullname=${authModel.fullname}');
